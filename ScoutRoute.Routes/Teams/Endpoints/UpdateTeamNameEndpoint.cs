...
{
    internal static class UpdateTeamNameEndpoint
    {
        public const string Name = "UpdateTeamName";


        public static IEndpointRouteBuilder MapUpdateTeamName(this IEndpointRouteBuilder app)
        {
            app
                .MapPut(Contracts.Endpoints.Endpoints.Teams.UpdateTeamName, async (Guid projectId, Guid teamId, UpdateTeamNameCommand command, IDocumentStore store, CancellationToken cancellationToken) =>
                {
                    try
                    {
                        ...
                        var ev = team.UpdateName(command.Name);
